# -*- coding: utf-8 -*-
"""OPTIC_DISC_TEST_FINAL_AUGMENTED_RESULT_(4) (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/13vg7n89sV___-K7KWXcypSLT6vVJ6L3R
"""
...
class DataGen(keras.utils.Sequence):
    ...
    def __load__(self, id_name, mask_id_name):
        image_path = os.path.join(self.path, "IMAGES_OK", id_name) + ".jpg"
        mask_path = os.path.join(self.path, "MASKS_PLS/", mask_id_name) + ".jpg"
...
class TestGen(keras.utils.Sequence):
    ...
    def __load__(self, id_name):
        image_path = os.path.join(self.path, "IMAGES_OK/", id_name) + ".jpg"
        mask_path = os.path.join(self.path, "MASKS_PLS/", id_name) + ".jpg"
...
# 30 original images, each with 7 samples (total of 210 images)

train_ids = ['IDRiD_{:03d}_{:d}'.format(i, j) for i in range(1, 25) for j in range(7)]  # 80% for training (168 images)
test_ids = ['IDRiD_{:03d}_{:d}'.format(i, j) for i in range(25, 28) for j in range(7)]  # 10% for testing (21 images)
valid_ids = ['IDRiD_{:03d}_{:d}'.format(i, j) for i in range(28, 31) for j in range(7)]  # 10% for validation (21 images)
...
model.save_weights("/content/drive/MyDrive/MulTiFINAL.h5")
...
def calculate_ssim(y_true, y_pred):
    ...
    # Check if the images are large enough
    if height < 7 or width < 7:
        print("Images are too small for SSIM calculation. Size:", (height, width))
        return None  # or return a suitable default value, e.g., 0 or np.nan

    # Calculate SSIM with a specified win_size (e.g., 11)
    ssim = compare_ssim(y_true, y_pred, data_range=y_pred.max() - y_pred.min(),
                        win_size=11, multichannel=True)
...
print("True Image Shape:", Y[0].shape)
print("Predicted Image Shape:", result[0].shape)
...
print("Average Dice Coefficient :", average_dice_coef)
...
def calculate_sensitivity(y_true, y_pred):
    ...
    TP = np.sum((y_true_flat == 1) & (y_pred_flat == 1))  # True Positives
    FN = np.sum((y_true_flat == 1) & (y_pred_flat == 0))  # False Negatives

    # Avoid division by zero
    if (TP + FN) == 0:
        return 0.0  # No positive cases
...
# Example usage
# Assuming Y is the ground truth and result is the predicted output
sensitivity_score = calculate_sensitivity(Y[0], result[0])  # Use the appropriate index for your data
